...
select
    coalesce(sum(health_check),0) [Uspješno u zadnja 24h],
    coalesce(sum(case when health_check = 0 AND last_backup IS NOT NULL then 1 else 0 end),0) [Uspješno ali starije od 24h],
    coalesce(sum(case when health_check = 0 AND last_backup IS NULL then 1 else 0 end),0) [Nema backupa]
